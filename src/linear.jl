...
function LinearModel(p::Integer, n::Integer)
    LinearModel(zeros(p),
                Array(Float64, p),
                Array(Float64, n),
                Array(Float64, n),
...
function predict!(m::LinearModel, X::Matrix{Float64})
    p, n = size(X)
    for i = 1:n
        m.pr[i] = dot(X[:, i], m.w)
    end
...
function residuals!(m::LinearModel, X::Matrix{Float64}, y::Vector{Float64})
    p, n = size(X)
    for i = 1:n
        m.r[i] = y[i] - m.pr[i]
    end
...
function fit_sgd(::Type{LinearModel},
                 ex::Expr,
